...
if ( isSecureConneciton() ) @ini_set( 'session.cookie_secure', 1 );
...
if ( php_sapi_name() === 'cli' ) {
  ...
  switch ( $cli['action'] ) {
    ...
    case 'get.info' :
      ...
      $cliGetInfo['freespace']     = disk_free_space( $sourcePath );
...
function cliPerformImport( $data )
{
  ...
  if ( $sourcePath === false ) {
    addWarning( "No installed website found.", 4 );
    showWarningJson( ['status' => 0, 'message' => ''], 1 );
  }
  $ACMS['ACMS_TIMEOUT'] = 0;
...
function deleteBackups( $params )
{
  ...
  if ( isset( $params['all'] ) ) {
    ...
    deleteDirectory( $sourcePath . DIRECTORY_SEPARATOR . 'backup' );
    createDirectory( 'backup' );
    createTable( 'backup' );
    return true;
  }
...
function downloadFromSerial( $uuid, $taskOffset = 0 )
{
  ...
  downloadFile(
    ...
    $sourcePath . DIRECTORY_SEPARATOR . 'imports' . DIRECTORY_SEPARATOR . $uuid . '.zip', $taskOffset
  );
  return $uuid;
}
...
function exportPerform( $exportZIP, $exportSettings, $taskOffset = 0 )
{
  ...
  if ( filesize( $exportTmpZIP ) == 0 ) {
    ...
    if ( !empty( $exportSettings['custom_files'] ) ) {
    ...
    if ( !empty( $exportSettings['templates'] ) ) {
  ...
  unlink( $exportTmpDB );
  createDirectory( 'imports' );
  rename( $exportTmpZIP, $sourcePath . DIRECTORY_SEPARATOR . 'imports' . DIRECTORY_SEPARATOR . $exportZIP );
  return true;
}
...
function funConvertWebsite()
{
  ...
  $pdo  = newPDO();
  $stmt = $pdo->query( "SELECT * FROM structure ORDER BY filetime" );
...
function removeBrokenLinks( $taskOffset = 0 )
{
  ...
  if ( !empty( unserialize( $taskStats ) ) ) {
    ...
    $stats = ['links' => 0, 'pages' => 0, 'processed' => 0, 'total' => 0, 'stats' => []];
  }
  if ( function_exists( 'libxml_use_internal_errors' ) ) libxml_use_internal_errors( true );
  $pdo = newPDO();
  if ( empty( $stats['total'] ) ) $stats['total'] = intval( $pdo->query( "SELECT COUNT(1) FROM structure WHERE mimetype = 'text/html'" )->fetchColumn() );
...
function removeVersionControl( $taskOffset = 0 )
{
  ...
  $replaceUrl = [
    ...
    '\?[\w]+$'                               => '',
    '\?(?:PHPSESSID|sid)=[a-z\d]+$'          => '',
    '\?(?:PHPSESSID|sid)=[a-z\d]+&(?:amp;)?' => '?',
    '&(?:amp;)?(?:PHPSESSID|sid)=[a-z\d]+'   => '',
  ];
  ...
  if ( !inSafeMode() ) {
    switch ( $_POST['action'] ) {
      ...
      case 'fun.convert.website.delete' :
        deleteDirectory( $sourcePath . DIRECTORY_SEPARATOR . 'export' );
        $section = 'tools';
        break;
      ...
      <div class="row h-100">
          ...
          <div class="p-1">
            ...
            <?php if ( inExpertMode() && count( $_POST ) ) { ?>
              ...
              <div class="rounded-bottom bg-dark mb-1">
                <div class="d-flex text-white text-monospace text-uppercase px-3 pb-3">
                  <div class="expand-label" data-toggle="collapse" href="#collapseDebug" role="button" aria-expanded="false" aria-controls="collapseDebug">
                    <i class="fas fa-caret-right fa-fw mr-1"></i><?=L( 'Debug' )?></div>
        ...
        <div class="col-12 col-lg-9 ml-auto pt-0 order-1 order-md-2 pb-3" id="main">
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="input-group">
                                    ...
                                    <div class="input-group-append">
                                      <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?=L( 'or select' )?></button>
                                      <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item put-custom-rule cursor-link" data-keyphrase="&lt;/head&gt;" data-regex="0" data-limit="1" data-position="-1"><?=L( 'Before' )?>
                                          <span class="text-monospace">&lt;/head&gt;</span></a>
                                        <a class="dropdown-item put-custom-rule cursor-link" data-keyphrase="&lt;/body&gt;" data-regex="0" data-limit="1" data-position="-1"><?=L( 'Before' )?>
                                          <span class="text-monospace">&lt;/body&gt;</span></a>
                                        <a class="dropdown-item put-custom-rule cursor-link" data-keyphrase="&lt;body[^&gt;]*&gt;" data-regex="1" data-limit="1" data-position="1"><?=L( 'After' )?>
                                          <span class="text-monospace">&lt;body&gt;</span></a>
                                      </div>
                                    </div>
                                  </div>
          ...
          <div class="row row-cols-1 row-cols-md-3">
                    ...
                    <form action="" method="post" id="form_update_external_links" class="form-wait">
                      <div class="row">
                        <div class="col-12">
                          <div class="form-group">
                            <label>rel</label>
                            <input class="form-control form-control-sm w-100" type="text" name="attributes[rel]" value="" placeholder="<?=L( 'Leave empty to skip' )?>">
                          </div>
                          <div class="form-group">
                            <label>target</label>
                            <input class="form-control form-control-sm  w-100" type="text" name="attributes[target]" value="" placeholder="<?=L( 'Leave empty to skip' )?>">
                          </div>
                        </div>
                ...
                <div class="card-body small">
                  ...
                  if ( !empty( $acmsCloudflare ) ) {
                    ...
                    <div class="alert-info p-3 my-3 rounded">
                      ...
                      Dev Mode: <?=( $devModeTimeRemaining > 0 ? getHumanTime( $devModeTimeRemaining ) : L( 'off' ) )?>
                      <br>
              ...
              <div class="col mb-4">
                <div class="card h-100 shadow">
                  ...
                  <div class="card-footer d-flex">
                    <?php if ( file_exists( $sourcePath . DIRECTORY_SEPARATOR . 'export' ) ) { ?>
                      <div>
                        <form action="" method="post">
                          <input type="hidden" name="xsrf" value="<?=$_SESSION['acms_xsrf']?>">
            ...
            <?php if ( false && !inSafeMode() && inExpertMode() ) { ?>
            <div class="col mb-4">
              <div class="card h-100 shadow">
                <form action="" method="post">
                  <div class="card-header text-white bg-info">
          ...
          if ( !empty( $fBucket['getImportsList'] ) ) foreach ( $fBucket['getImportsList'] as $brokenImport ) { ?>
            <div class="bg-light rounded border shadow p-3 mt-2 mb-1">
              <div class="row">
                ...
                <div class="col-12 col-md-3">
                  <?php if ( false && inExpertMode() ) { ?>
                    <form action="" method="post" class="needs-validation" novalidate>
                      <div class="form-group">
                        <label for="import_funmd_hostname"><?=L( 'Domain' )?></label>
                        <input class="form-control" type="text" name="params[hostname]" placeholder="<?=L( 'Domain' )?>" id="import_funmd_hostname" required>
                      </div>
                      <input type="hidden" name="xsrf" value="<?=$_SESSION['acms_xsrf']?>">
          ...
          <div class="bg-light rounded border shadow-sm p-3">
            <form action="" method="post" class="needs-validation form-wait" novalidate>
              ...
              <div class="input-group">
                ...
                <input type="text" class="form-control" name="filename" placeholder="<?=L( 'Enter a .ZIP file name' )?>" value="<?=$uuidSettings['uuid'] . '-' . date( 'YmdHis' ) . '.zip'?>" required>
                <div class="input-group-append">
            ...
            <div class="bg-light border rounded p-3 mb-2 search-result">
              <div class="row">
                ...
                <div class="col-sm-auto col-12 text-center text-sm-right text-nowrap order-1 order-sm-2 mb-3 mb-sm-0">
                  ...
                  <?php if ( inExpertMode() ) { ?>
                    <a class="btn btn-sm btn-info" target="_blank" rel="noreferrer noopener" href="https://web.archive.org/web/<?=$searchResult['filetime']?>/http://<?=$searchResult['hostname']?><?=$searchResult['request_uri']?>">
                      <i class="fas fa-landmark fa-fw"></i>
                    </a>
                    <div class="small text-monospace text-center">
                      <span class="small badge badge-info"><?=convertFiletimeToHuman( $searchResult['filetime'] )?></span>
                    </div>
                  <?php } ?>
                </div>
              </div>
                  ...
                  <div class="row">
                    <div class="col-12 col-md-6">
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                          <input type="text" class="form-control form-control-sm" name="charset" pattern="[-a-z0-9_]*" value="<?=htmlspecialchars( $metaData['charset'] )?>">
                        </div>
                      </div>
                            ...
                            <?php if ( inExpertMode() ) { ?>
                              <div class="input-group-append">
                                <span class="input-group-text bg-info text-light">
                                  <a class="text-light" href="https://web.archive.org/web/<?=htmlspecialchars( $metaData['filetime'] ) . '/' . htmlspecialchars( $metaData['url'] )?>" target="_blank" rel="noopener noreferrer">
                  ...
                  <table class="table table-sm table-hover mb-3">
                    ...
                    <tr>
                      ...
                      <td class="text-monospace"><?=implode( '-', str_split( $uuidSettings['uuid'], 4 ) )?>
                        <a href="https://archivarix.com/<?=getLang()?>/status/<?=( $uuidSettings['uuidg'] ? $uuidSettings['uuidg'] : $uuidSettings['uuid'] )?>/" target="_blank" rel="nofollow noopener"><i class="fas fa-external-link-alt fa-fw"></i></a>
                      </td>
                    </tr>
                    <tr>
                      <td><?=L( 'Created' )?></td>
          ...
          <div class="modal-body">
            ...
            <div class="collapse hide mt-2" id="createNewUrlAdditional">
              <div class="form-group">
                ...
                <input type="text" name="charset" class="form-control" id="newUrlCharset" placeholder="<?=L( 'Enter a charset if required' )?>" pattern="[-a-zA-Z0-9/_]*">
              </div>
              <div class="form-group">
                <label for="newUrlHostname"><?=L( 'Domain' )?></label>
                <input type="text" name="hostname" class="form-control" id="newUrlHostname" placeholder="<?=L( 'Enter a domain name' )?>" required>
              </div>
            </div>
          </div>
          ...
          <form action="" method="post" class="needs-validation" novalidate>
            ...
            <div class="modal-body">
              <div class="form-group">
                ...
                <input type="text" class="form-control mb-2 px-3 clear" name="request_uri" id="cloneUrlPath" value="<?=htmlspecialchars( rawurldecode( $metaData['request_uri'] ), ENT_IGNORE )?>" required/>
              ...
              <div class="collapse hide mt-2" id="cloneNewUrlAdditional">
                <div class="form-group">
                  ...
                  <input type="text" name="hostname" class="form-control" id="cloneUrlHostname" placeholder="<?=L( 'Enter a domain name' )?>" value="<?=htmlspecialchars( convertIdnToUtf8( $metaData['hostname'] ) )?>" required>
                </div>
              </div>
              <input type="hidden" name="xsrf" value="<?=$_SESSION['acms_xsrf']?>">
...
<?php if ( in_array( $section, ['table', 'missing.urls', 'backups'] ) ) { ?>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.18.0/locale/bootstrap-table-<?=$cmsLocales[getLang()]['code']?>.min.js" crossorigin="anonymous"></script>
<?php } ?>
...
<?php if ( in_array( $section, ['settings', 'template', 'htaccess'] ) || isset( $metaData['rowid'] ) ) { ?>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/addon/edit/matchtags.min.js" integrity="sha512-q/FQZQVZpwUkYxhjfZZekmH74gSxVZ64hVJ16fVAn61bl/vW+vGGNs5NzLphNop0p/Xo/hWLCvnAXIFIiWOVfQ==" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/addon/search/jump-to-line.min.js" integrity="sha512-99Q4HwQrLquE8B4tjv3R5fL2/NABD0JkKppzE4keanuKWwibhrlXRDehr9FiG1aq0FLyO1TUOvSGGz1cgtSANg==" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/addon/search/search.min.js" integrity="sha512-Scy8gOuTrwkguZyL89xZe5MMIdgZGUUQAkkDKtUI4mq2fNVTRg+pSsPcXkRtNmFN8sQ23MZNA0FsqJUPEQ0PUQ==" crossorigin="anonymous"></script>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/addon/selection/active-line.min.js" integrity="sha512-ysQeDEwbdvERZqZCqFd64rVjSx4ExrC/r581h40cMF4e6rFWS6VxvdVxmSf/cLr+oe9mVxxzWSMhPJYSFyiVew==" crossorigin="anonymous"></script>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/mode/php/php.min.js" integrity="sha512-m8sosGXUwyH6Ppzoy+CoQ/r5zAwZRGdNFUgGH81E3RDQkFnAsE4cP1I3tokvZwgMsDZB5mHxs+7egAgvhaCcMw==" crossorigin="anonymous"></script>
...
<script language="JavaScript">
  ...
  var submitNonAjax = function formSubmitNonAjax(form_id) {
    $('#savedAlert').html('<?=htmlspecialchars( L( 'Sending with AJAX failed. Sending data the regular way' ), ENT_QUOTES )?>&hellip;');
  ...
  $('#checkUpdate').on('click touch', function () {
    $.getJSON("https://archivarix.com/<?=getLang()?>/cms/?ver=<?=ACMS_VERSION?>&uid=<?=sha1( __DIR__ )?>", function (json) {
